...
export const AdminDashboard = ({ user }) => {
  ...
  const ViewPermissions = ({ user }) => {
    const userPermissions = permissions[user.role]?.permissions || [];
    return (
      <div style={{
        background: C.white,
        borderRadius: 12,
        ...
        <h3 style={{ marginBottom: 16, color: "#111827" }}>Kebenaran Anda</h3>
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {userPermissions.map((perm, i) => (
            <div key={i} style={{
              ...
              gap: 8,
              padding: "6px 0",
              borderBottom: i < userPermissions.length - 1 ? "1px solid #F3F4F6" : "none"
            }}>
              <span style={{ color: "#22C55E", fontSize: 14 }}>✓</span>
              <span style={{ fontSize: 13, color: "#374151" }}>{perm}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  // UC005: Assign role to user
  const AssignRoleModal = ({ user, onClose, onAssign }) => {
    const [selectedRole, setSelectedRole] = useState(user.role);

    const handleAssign = () => {
      if (selectedRole === user.role) {
        showToast("Pengguna sudah mempunyai peranan ini.", "warning");
        return;
      }
      onAssign(user.id, selectedRole);
      showToast(`Peranan dikemaskini kepada ${roleLabels[selectedRole]} untuk ${user.name}.`);
      onClose();
    };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 20 }}>
            Menetapkan peranan kepada <strong>{user.name}</strong>. Perubahan berkuat kuasa serta-merta.
          </p>
          ...
          {Object.entries(roleLabels).map(([roleKey, roleLabel]) => (
            <div
              ...
              style={{
                ...
                marginBottom: 8,
                borderRadius: 10,
                border: selectedRole === roleKey ? "2px solid #2563EB" : "1px solid #E5E7EB",
                background: selectedRole === roleKey ? "#EFF6FF" : C.white,
                cursor: "pointer",
                display: "flex",
                alignItems: "center",
                gap: 12,
            ...
            >
              <div style={{
                ...
                height: 20,
                borderRadius: "50%",
                border: `2px solid ${selectedRole === roleKey ? "#2563EB" : "#D1D5DB"}`,
                background: selectedRole === roleKey ? "#2563EB" : C.white,
                display: "flex",
                alignItems: "center",
                justifyContent: "center"
              }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: "#6B7280" }}>
                  {permissions[roleKey]?.permissions?.length || 0} kebenaran
                </div>
              </div>
              <span style={{
                padding: "2px 10px",
                ...
                fontSize: 10,
                fontWeight: 600,
                background: getRoleColor(roleKey).bg,
                color: getRoleColor(roleKey).text
              }}>
                {roleKey}
              </span>
            </div>
  ...
  // UC007: Revoke user access
  const RevokeAccessModal = ({ user, onClose, onRevoke }) => {
    const [reason, setReason] = useState('');

    const handleRevoke = () => {
      if (!reason.trim()) {
        showToast("Sila berikan sebab untuk batalkan akses.", "warning");
        return;
      }
      onRevoke(user.id, reason);
      showToast(`Akses dibatalkan untuk ${user.name}.`, "error");
      onClose();
    };
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <h3 style={{ textAlign: "center", marginBottom: 8, color: "#111827" }}>
            Batalkan akses {user.name}?
          </h3>
          <p style={{ fontSize: 13, color: "#6B7280", textAlign: "center", marginBottom: 20, lineHeight: 1.6 }}>
            Ini melumpuhkan akaun serta-merta. Pengguna tidak dapat log masuk sehingga admin memulihkan akses.
            Gunakan untuk kes peletakan jawatan atau pelanggaran keselamatan.
          </p>
          ...
          <label style={{
            ...
            display: "block",
            marginBottom: 4
          }}>Sebab (untuk audit)</label>
          <textarea
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            placeholder="Contoh: Peletakan jawatan / Pelanggaran keselamatan"
            rows={3}
            style={{
              width: "100%",
              padding: "10px 14px",
  ...
  // UC006: Set permissions per role
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 20 }}>
            Konfigurasi kebenaran global untuk setiap peranan. Perubahan berkuat kuasa untuk semua pengguna dalam peranan tersebut.
          </p>

          {Object.entries(tempPermissions).map(([roleKey, roleData]) => (
              ...
              <div style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#6B7280" }}>
                    {roleData.permissions.length} kebenaran
                  </div>
                </div>
                <span style={{
                  padding: "2px 12px",
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
            ...
            <button
              onClick={() => {
                ...
                showToast("Kebenaran peranan berjaya dikemaskini!", "success");
                onClose();
              }}
              style={{
                flex: 1,
    ...
    <div style={{ padding: "32px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      {toast && (
        <div style={{
          ...
          padding: "12px 24px",
          fontSize: 14,
          color: toast.type === "error" ? "#DC2626" : toast.type === "warning" ? "#92400E" : "#166534",
      ...
      <div style={{ display: "flex", gap: 4, background: "#F3F4F6", padding: 4, borderRadius: 12, marginBottom: 24 }}>
        {tabs.map(tab => (
          ...
          >
            ...
            {tab.count && (
              <span style={{
                background: activeTab === tab.id ? "#EFF6FF" : "#E5E7EB",
                color: activeTab === tab.id ? C.blue : "#6B7280",
                padding: "1px 8px",
                borderRadius: 12,
                fontSize: 11,
                fontWeight: 600
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ fontSize: 15, fontWeight: 600, color: "#111827" }}>{u.name}</span>
                      <span style={{
                        padding: "2px 10px",
                        borderRadius: 12,
                        fontSize: 10,
                        fontWeight: 600,
                        background: getStatusColor(u.status).bg,
                        color: getStatusColor(u.status).text
                      }}>
                        {u.status}
                      </span>
                    </div>
                    ...
                    <div style={{ marginTop: 4 }}>
                      <span style={{
                        ...
                        background: getRoleColor(u.role).bg,
                        color: getRoleColor(u.role).text,
                        border: `1px solid ${getRoleColor(u.role).border}`
                      }}>
      ...
      {activeTab === 'audit' && (
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontWeight: 600, color: "#111827" }}>Log Aktiviti</span>
            <button style={{
              padding: "6px 16px",
              borderRadius: 8,
              border: "1px solid #E5E7EB",
